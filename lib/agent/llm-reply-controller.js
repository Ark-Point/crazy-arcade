...
function actionSnapshot(action) {
  ...
  const snapshot = {
    type: typeof action.type === 'string' ? action.type.slice(0, 32) : 'unknown',
  };
  if (Number.isSafeInteger(action.seq)) snapshot.seq = action.seq;
  if (typeof action.direction === 'string') snapshot.direction = action.direction.slice(0, 16);
...
function policyFromReply(reply, revision, decisionTick, heuristicId, sequencePlan, action) {
  ...
  return {
    ...
    lastAction: actionSnapshot(action),
...
function createLlmReplyController(options = {}) {
  ...
  function emitDecision(observation, reply, heuristicId) {
    ...
    const policy = policyFromReply(reply, revision, decisionTick, heuristicId, movementSequencer.currentPlan(), action);
